...
/*   Updated: 2021/07/01 12:55:22 by dboyer           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 */
static void _initBind(int fd, struct sockaddr_in *address,
                      socklen_t socklen) throw(Socket::SocketException) {
...
Socket::Socket(void) throw(Socket::SocketException)
    : _fd(socket(AF_INET, SOCK_STREAM, 0)), _opt(1),
...
Socket::Socket(const Socket &other) throw(Socket::SocketException)
    : _fd(dup(other._fd)), _opt(other._opt), _address(other._address),
 ...
 */
std::string Socket::get_request() const { return _request; }
...
t_locationData *Socket::get_locationData(void) const { return _loc; }

std::vector<std::string> Socket::get_infos(void) const { return _infos; }
...
void Socket::listen(const int port,
                    const std::string addr) throw(Socket::SocketException) {
...
void Socket::headerCode(std::string content, int code, t_serverData data) {
  if (!data.error_page[code].empty() &&
      data.error_page[code] == "error_page_not_valid" && code != 404) {
    !data.error_page[404].empty() ? _content = data.error_page[404]
                                  : _content = "<h1>" + content + "</h1>";
    _code = "404 Not Found";
    return;
  }
  if (!data.error_page[code].empty() &&
      data.error_page[code] == "error_page_not_valid") {
    _content = "<h1>" + content + "</h1>";
    _code = content;
    return;
  }
  !data.error_page[code].empty() ? _content = data.error_page[code]
                                 : _content = "<h1>" + content + "</h1>";
  _code = content;
}
...
void Socket::directoryListing(std::string file, t_serverData data) {
  ...
  if (!(dh = opendir(directory.c_str())))
    ...
    while ((contents = readdir(dh)) != NULL) {
      if ((is_dir =
               opendir((data.root + _infos[1] + std::string(contents->d_name))
                           .c_str())))
        d_slash = "/";
      closedir(is_dir);
      if (std::string(contents->d_name) != ".")
        _content +=
            ("<li><a href=\"" + std::string(contents->d_name) + d_slash +
...
std::string Socket::Cgi(t_serverData &data) {
  ...
  if ((pid = fork()) == 0) {
    ...
    ::close(fd[0]);
    ::close(fd[1]);
    execl("cgi-bin/php-cgi", "cgi-bin/php-cgi", ("www" + _infos[1]).c_str(),
          NULL);
...
void Socket::locAutoindex(t_serverData data) {
  ...
  for (std::list<t_locationData>::iterator it = data.locations.begin();
       it != data.locations.end(); ++it) {
    path1 = (it->path[it->path.size() - 1] == '/')
                ? it->path.substr(0, it->path.size() - 1)
                : it->path;
...
void Socket::locIndex(t_serverData data) {
  ...
  for (std::list<std::string>::iterator it = data.index.begin();
  ...
  for (std::list<t_locationData>::iterator it1 = data.locations.begin();
       it1 != data.locations.end(); ++it1) {
    path1 = (it1->path[it1->path.size() - 1] == '/')
                ? it1->path.substr(0, it1->path.size() - 1)
                : it1->path;
    if (!it1->index.front().empty() &&
        _infos[1].find(path1) != std::string::npos) {
      for (std::list<std::string>::iterator it2 = it1->index.begin();
           it2 != it1->index.end(); ++it2) {
...
bool Socket::methodAllowed(t_serverData data) {
  ...
  for (std::list<t_locationData>::iterator it = data.locations.begin();
    ...
    path1 = (it->path[it->path.size() - 1] == '/')
                ? it->path.substr(0, it->path.size() - 1)
                : it->path;
    if (_infos[1].find(path1) != std::string::npos &&
        contains(it->methods, _infos[0]))
      return false;
  }
  return true;
}
...
bool Socket::operator==(const int fd) const { return fd == _fd; }

bool Socket::operator==(const Socket &other) const { return _fd == other._fd; }

bool operator==(const int fd, const Socket &s) { return fd == s.Fd(); }
